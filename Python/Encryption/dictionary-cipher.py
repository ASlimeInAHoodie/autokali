...
if ((not launch_builder) and dictionary != '0' and dictionary != '1'):
    print("Ξ CUSTOM Ξ DICTIONARY Ξ IDENTIFIED Ξ")
...
def encrypt(text, dictionary, seperator):
    ...
    invalid_char = 0
    for i in text:
        enc_text += str(str(dictionary).find(i)) + seperator
        # Check if there are invalid characters
        if str(str(dictionary).find(i)) == "-1":
            invalid_char += 1
            # Raise an error if 20% or more of characters are invalid. This is roughly the most that can be removed before text becomes unreadable. 
            if (invalid_char >= (len(text) * 0.2)):
                raise InputError("Too many characters are invalid for the decrypted version to be readable, please use a different dictionary.")
    # Raise warning if there are invalid characters
    if (invalid_char > 0):
        print("WARNING: Character/s do/es not appear in the dictionary.")
    return enc_text
